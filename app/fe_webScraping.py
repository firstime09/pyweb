...
def app():
    ...
    start_date = st.date_input(
        "Select start date to filter reviews from:",
        value=None)
    ...
    if st.button("Fetch App Data"):
        try:
            ...
            clean_reviews = []
            for rv in rvs:
                review_date = rv.get("at", None)  # datetime object
                if start_date is None or (review_date and review_date.date() >= start_date):
            ...
            st.write(f"Total reviews fetched and filtered: {len(df_reviews)}")
...
#             st.subheader("Reviews")
#             st.dataframe(df_reviews)
#             st.write(f"Total reviews fetched: {len(df_reviews)}")
